...
<%
	...
	strCourseName = request.getParameter("courseName");//URLDecoder.decode(request.getParameter("courseName"), "UTF-8");
	strWeekDay = request.getParameter("weekDay");
	strNumber = request.getParameter("number");
	strTeacher = request.getParameter("teacher");
	strPlace = request.getParameter("place");
	strCredit = request.getParameter("credit");
	...
	try {
		...
		if (con.isClosed()) {
			...
			String query = "INSERT INTO course (courseName, weekDay, number,teacher,place,credit) VALUES ('"
					+ strCourseName + "','" + strWeekDay + "','" + strNumber + "','" + strTeacher + "','"
					+ strPlace + "'," + strCredit + ")";
			//out.println(query);
